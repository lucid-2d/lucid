...
export class ResultScene extends SceneNode {
  constructor(private app: App, private score: number) {
    super({
  ...
  onEnter() {
    this.addChild(new Label({ text: 'Game Over', fontSize: 32, fontWeight: 'bold', color: UIColors.accent, align: 'center', width: 300, height: 40 }));
    ...
    this.addChild(new Label({ text: 'Score', fontSize: 16, color: UIColors.textMuted, align: 'center', width: 300, height: 24 }));

    // Spacer
    this.addChild(new UINode({ width: 1, height: 20 }));

    const againBtn = new Button({ id: 'again', text: 'Play Again', variant: 'primary', width: 200, height: 50 });
    againBtn.$on('tap', () => this.app.router.replace(new GameScene(this.app)));
