...
| `auditUX(app, opts?)` | function | **UX audit engine.** Runs 26 built-in rules (touch targets, safe area, navigation, overlap, emoji, components, etc.). Returns `{ pass, issues, summary, layout }`. Options: `{ ignore?, rules?, strict? }`. When `strict: true`, warnings also cause `pass=false`. |
| `auditAll(app, entries, opts?)` | function | **Batch audit multiple scenes.** Takes `{ scene, label, setup? }[]`, returns `{ allPassed, sceneCount, perScene, summary }`. Passes `opts` (including `strict`) through to `auditUX`. |
